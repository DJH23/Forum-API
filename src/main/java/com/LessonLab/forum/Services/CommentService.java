...
public class CommentService extends ContentService {
     ...
     * return contents.stream().map(content -> (Comment)
     ...
     * return new PageImpl<>(contents.getContent().stream().map(content -> (Comment)
    ...
    /*
     * public List<Comment> getCommentsByCreatedAtBetween(LocalDateTime start,
     * LocalDateTime end) {
